import React from "react";


const Enroll = () => {
  return (
    <div className="mt-8 w-full h-screen flex items-center justify-center">
     <div className="w-[90%]">
        <h4 className="text-center text-sm font-bold">Testmonials</h4>
        <h2 className="text-center text-4xl font-bold">See What Our Students Say</h2>
        <h3 className="mt-8 w-full md:w-[60%] text-center text-xl font-semibold mx-auto">"The Microdome classes transformed my preparation journey! Their 
        resources and support were invaluable in my success."</h3>
        <div className="w-16 h-16 rounded-full mt-4 bg-gray-300 mx-auto"></div>
        <p className="mt-2 text-lg text-center mx-auto font-bold">Sourav Ghosh</p>
        <p className="text-sm text-center mx-auto">M.Sc in Biotechnology , IIT Delhi</p>
     </div>
    </div>
  );
};
 
export default Enroll;
