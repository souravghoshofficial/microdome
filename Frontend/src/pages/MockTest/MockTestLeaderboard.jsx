...
//           Attempt {user.bestAttempt.attemptNumber} | Score: {user.bestAttempt.score} | Time: {formatTime(user.bestAttempt.timeTakenSeconds)}
//         </div>
//       </div>
  ...
  return (
    <div className="flex flex-col items-center w-20 sm:w-24 md:w-32 shrink-0">
      ...
      {user.attempts.length > 1 && (
        <button
          onClick={() => setOpen(!open)}
          className="mt-1 flex items-center gap-1 text-[10px] sm:text-xs text-blue-600 dark:text-blue-400"
        >
          Attempts
          <ChevronDown
            className={`w-3 h-3 transition ${
      ...
      <tr className="border-t border-gray-200 dark:border-gray-800">
        ...
        <td className="p-3 text-right">
          {u.attempts.length > 1 && (
            <button
              onClick={() => setOpen(!open)}
              className="flex items-center gap-1 text-xs sm:text-sm text-blue-600 dark:text-blue-400"
            >
              Attempts
              <ChevronDown
                className={`w-4 h-4 transition ${
    ...
    >
      <div className="max-w-5xl mx-auto px-3 sm:px-4 py-8 sm:py-12">
        <div className="mb-6">
          ...
          {meta && (
            <div className="mt-3 flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-4 text-xs sm:text-sm text-gray-600 dark:text-gray-400">
              <div className="flex items-center gap-1">
                <Hash className="w-4 h-4" />
                {meta.mockTestType}
              </div>
